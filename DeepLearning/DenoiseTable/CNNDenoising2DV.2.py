...
def load_data_from_npz(npz_path="./DenoisingDataTransSheet10.npz"):
    """
    ...
    Returns:
        tuple: A tuple containing the two noisy histograms, a dummy clean target
               (for compatibility with later plotting functions), a mask, and
               the dimensions and coordinate grids.
    """
...
def create_cnn_inputs_pytorch(noisy_input_data, noisy_target_data, clean_mask_data, ct_vals, initial_e_vals, num_a, num_e):
    ...
    for m_idx in range(noisy_input_data.shape[0]):
        for e_idx in range(noisy_input_data.shape[1]):
            ...
            y_target_images.append(noisy_target_data[m_idx, e_idx, :, :].unsqueeze(0))
            mask_images.append(clean_mask_data[m_idx, e_idx, :, :].unsqueeze(0).float())

    return torch.stack(X_cnn_images, dim=0), torch.stack(y_target_images, dim=0), torch.stack(mask_images, dim=0)


# --- 4. Robust U-Net Architecture with Residual and Attention Blocks ---
...
class DenoisingUNet(nn.Module):
    def __init__(self, in_channels=5, out_channels=1, features_list=None):
        ...
        # Decoder Path with Attention & Residual Blocks
...
def plot_distributions_before(noisy_input_data, noisy_target_data, m_indices, e_indices, ct_vals, initial_e_vals, x_angles_np, y_final_energies_np):
    ...
    for m_idx_to_plot in m_indices:
        for e_idx_to_plot in e_indices:
            ...
            plt.savefig(f"histogram_example_M{m_idx_to_plot}_E{e_idx_to_plot}.png") # Uncomment to save figures
            # plt.show()
...
if __name__ == "__main__":
    ...
    # Create a mutually exclusive group for dummy or real data
    ...
    plot_distributions_before(
        noisy1_histograms, noisy2_target, 
        m_indices_to_plot_final, e_indices_to_plot_final, 
        ct_values, initial_energies, 
        x_angles_range, y_final_energies_range
    )


    # Scale the data by amplitude_scaling_factor
