...
# Squeeze-and-Excite block (remains the same)
...
# Basic Conv3D block with optional SE and Residual Connection (remains the same)
class ConvBlock3D(nn.Module):
    def __init__(self, in_c, out_c, se=False):
        ...
        self.final_relu = nn.LeakyReLU(negative_slope=0.01, inplace=True) # Using LeakyReLU for better gradient flow
...
# Full 3D U-Net (remains the same)
class Noise2Noise3DUNet(nn.Module):
    def __init__(self, in_channels=4, base_channels=32):
        ...
        # Output layer: predict noise (NO ReLU here, noise can be negative)
...
# # Full 3D U-Net (updated MaxPool3d to AvgPool3d)
# class Noise2Noise3DUNet(nn.Module):
#     def __init__(self, in_channels=4, base_channels=32):
#         super().__init__()
...
#         self.enc2 = ConvBlock3D(base_channels, base_channels * 2, se=True)
#         self.enc3 = ConvBlock3D(base_channels * 2, base_channels * 4, se=True)
#         self.pool = nn.AvgPool3d(2) # Changed from MaxPool3d to AvgPool3d
...
# DenoisingDataset class (updated to use constant padding for data)
class DenoisingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(f'Shape of noisy_patch slice before padding: {noisy_patch_slice_ch.shape} after padding: {noisy_patch.shape}')
...
# denoise_volume function (updated parameter names)
def denoise_volume(
    model,
    noisy_full_volume, # Renamed from noisy_volume
    patch_size,
    device='cuda',
    overlap_factor=0.5, # Renamed from overlap_ratio
    batch_size=4 # Added batch_size for inference
):
    """
    Denoises a full 3D volume by processing it in overlapping patches using the trained Noise2Noise model.
    Handles coordinate volume generation and reconstruction, applying final clamping for probabilities.

    Args:
        model (nn.Module): The trained Noise2Noise3DUNet model.
        noisy_full_volume (torch.Tensor): The full 3D noisy volume (D, H, W). Expected values [0, 1].
    ...
    with torch.no_grad():
        for i in tqdm(range(num_batches), desc="Denoising Patches"):
            ...
            # Predict noise
            predicted_noise_batch = model(batch_input) # (B, 1, D, H, W)

            # Denoise the batch: Subtract predicted noise from original noisy batch
            # Extract noisy part from batch_input (which is [noisy, coords])
            original_noisy_batch = batch_input[:, 0:1, :, :, :] # (B, 1, D, H, W)
            denoised_batch_output = original_noisy_batch - predicted_noise_batch
            ...
            for j, patch_output in enumerate(denoised_batch_output):
                original_noisy_patch_for_reconstruct = original_noisy_batch[j].cpu()
    ...
    # IMPORTANT: Clamp the final output to [0, 1] for probabilities
...
def train_noise2noise(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (noisy_patch, target_patch, mask_patch, coord_patch) in train_bar:
            ...
            # Model outputs the predicted noise
            predicted_noise = model(input_patch)

            # The target for the predicted noise is the difference between
            # the noisy input and the (noisy or clean) target patch.
            target_for_loss = noisy_patch - target_patch

            # Base loss (MSE)
            loss = mse_loss(predicted_noise, target_for_loss)

            # Add optional L1
            if "l1" in loss_type:
                # Use L1 loss on the difference (predicted noise vs target noise)
                loss += 0.1 * l1_loss(predicted_noise, target_for_loss)

            # Add optional Total Variation (TV) - applied to the predicted noise for smoothness
            if "tv" in loss_type:
                loss += 0.01 * total_variation_loss(predicted_noise)
...
def plot_2d_histograms(
    ...
    extent = (energyRange[0], energyRange[1], angleRange[0], angleRange[1]) # Note: imshow transposes by default, so order is (left, right, bottom, top)
...
if __name__ == "__main__":
    ...
    model = Noise2Noise3DUNet(in_channels=4, base_channels=32)
