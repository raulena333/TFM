...
def save_denoised_npz(denoised_4d, output_filename, hu_values, initial_energies, method, extra=None):
    ...
    """
    output_filename = f"{output_filename}.npz"
...
def plot_denoising_results_from_npz(histograms_mmap, denoised_4d, hu_values, initial_energies, method, plot_indices, directory="./plots", original_4d_shape=None):
    ...
    # Use the provided original_4d_shape
    ...
    for idx in plot_indices:
        ...
        # Correct title for the first plot
        ...
        # Correct title for the third plot
        axes[2].set_title('Reference Clean')
        ...
        plt.tight_layout()
        plt.savefig(f'{directory}/denoising_example_M{m_idx_to_plot}_E{e_idx_to_plot}.pdf')
        plt.close(fig)
    print(f"Generated {len(plot_indices)} denoising plots in {directory}.")
...
if __name__ == "__main__":
    ...
    # 1. Data Loading and Initial Setup (outside the loop)
    ...
    # 3. Define the hyperparameter grid to search over
    # This grid is designed to test a variety of combinations within a reasonable timeframe.
    kl_lambdas = [1000.0, 5000.0, 10000.0]
    fp_lambdas = [0.01, 0.1, 1.0, 10.0]
    fn_lambdas = [0.01, 0.1, 1.0]
    
    # Create all possible combinations
    param_combinations = list(itertools.product(kl_lambdas, fp_lambdas, fn_lambdas))

    print(f"[INFO] Starting hyperparameter search with {len(param_combinations)} combinations.")
    print("-" * 30)

    # Loop over each combination of hyperparameters
    for kl_l, fp_l, fn_l in param_combinations:
        
        # Create a unique name for this run based on the hyperparameters
        run_name = f"kl_{kl_l}_fp_{fp_l}_fn_{fn_l}"
        print(f"\n[INFO] Starting new run with parameters: {run_name}")
        
        # 4. Model Parameters and Training
        model_params = {
            'in_channels': 5,
            'out_channels': 1,
        ...
        # Train the model
        denoising_model.train_model(
            train_loader, val_loader, num_epochs=100, early_stopping_patience=10
        )

        # 5. Denoise the full dataset and plotting
        ...
        # Save the denoised data with the unique run name
        # save_denoised_npz(denoised_4d_data, f"denoised_output_{method}_{run_name}", ct_values, initial_energies, method, extra)
        ...
        # Create a specific directory for the plots of this run
        plot_dir = f"./plots/{run_name}"
        
        plot_denoising_results_from_npz(
            histograms_mmap, denoised_4d_data, ct_values, initial_energies, method, plot_indices, directory=plot_dir, original_4d_shape=original_4d_shape
        )
    
    print("\n[INFO] All hyperparameter tuning runs finished.")