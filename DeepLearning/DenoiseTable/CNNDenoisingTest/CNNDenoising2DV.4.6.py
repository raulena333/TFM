...
# --- 6. Custom Zero-Aware Hybrid Loss (with L1 regularization) ---
class ZeroAwareHybridLoss(nn.Module):
    def __init__(self, mse_weight=0.1, kl_weight=1.0, l1_weight=0.001, eps=1e-12):
    ...
    def forward(self, inputs, targets):
        ...
        # Flatten inputs/targets for easy calculation
        ...
        # --- Sparsity Regularization (L1 Loss) ---
        # This term is calculated on all outputs, encouraging sparsity everywhere
        l1_loss = torch.mean(torch.sum(torch.abs(flat_inputs), dim=1))
...
if __name__ == "__main__":
    ...
    print(f'\t.L1 Weight: {training_params["l1_weight"]}')
