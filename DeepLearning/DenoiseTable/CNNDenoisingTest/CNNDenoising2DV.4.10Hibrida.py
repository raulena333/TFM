...
# --- 6. Rewritten Loss Function: Simple KL Divergence ---
# class SimpleKLLoss(nn.Module):
#     def __init__(self):
#         super(SimpleKLLoss, self).__init__()
#         self.kl_loss = nn.KLDivLoss(reduction='batchmean', log_target=False)
#         print("[INFO] Initializing SimpleKLLoss with KLDivLoss.")

#     def forward(self, inputs, targets):
#         # We need to normalize both inputs (model logits) and targets (ground truth)
#         # to sum to 1 before calculating the KL divergence.
        
#         # 1. Normalize targets to a probability distribution
#         # The target values are not probabilities, they are scaled counts.
#         # We must normalize them to sum to 1.
#         targets_sum = targets.sum(dim=[1, 2, 3], keepdim=True)
#         # Add a small epsilon to avoid division by zero
#         normalized_targets = targets / (targets_sum + 1e-12)

#         # 2. Convert model outputs (logits) to log-probabilities
#         # This is the expected format for the `log_softmax` input
#         log_softmax_inputs = F.log_softmax(inputs.flatten(start_dim=1), dim=1)
        
#         # 3. Calculate KL Divergence
#         # The KLDivLoss expects log-probabilities for the input and probabilities for the target
#         loss = self.kl_loss(log_softmax_inputs, normalized_targets.flatten(start_dim=1))

#         return loss

class HybridKLLoss(nn.Module):
    """
    Combines KL Divergence for signal regions and L1 loss for sparsity in background.
    """
    def __init__(self, lambda_sparse=1.0, downsample_factors=None, reduction='batchmean'):
        """
        Args:
            lambda_sparse (float): Weight for the L1 sparsity penalty.
            downsample_factors (list): Optional list of factors for multi-scale loss.
        ...
        print(f"[INFO] Initializing HybridKLLoss.")
    ...
    def forward(self, inputs, targets):
        ...
        Args:
            inputs (torch.Tensor): Model outputs (raw logits), shape (B, 1, H, W).
        ...
        # The KL divergence is applied globally to encourage overall shape learning.
        
        # 1. Normalize the targets to be a valid probability distribution
        ...
        # --- Sparsity Penalty Component (L1 on Background) ---
        # This penalizes non-zero values in regions where the target is zero.
        
        # 1. Create a background mask (where targets are exactly zero)
        background_mask = (targets == 0).float()
        
        # 2. Extract the model's un-softmaxed logits for the background regions
        # We take the absolute value as L1 loss is a sum of absolute differences from zero.
        background_logits = inputs * background_mask
        
        # 3. Calculate the L1 loss on these background logits
        # We want to push the logits towards a very low value, close to zero,
        # which results in a near-zero probability after softmax.
        ...
        # Add multi-scale KL loss (optional but good practice)
        for factor in self.downsample_factors:
            ...
            # Normalize the downsampled targets
            ...
            # Compute KL loss on downsampled data
            downsampled_inputs_log_softmax = F.log_softmax(
                downsampled_inputs.flatten(start_dim=1), dim=1
            )
            total_loss += self.kl_loss(downsampled_inputs_log_softmax, downsampled_targets_norm.flatten(start_dim=1))
...
if __name__ == "__main__":
    ...
    training_params = {
        ...
        'lambda_sparse': 0.00000001, # 0.0000001
