...
def load_data(noisy_file_path, denoised_file_path, threshold=1e-4):
    ...
    try:
        ...
        noisy_data = noisy_npz['histograms'][:, :20 , :, :, :]  # Limit to first 20 initial energies for memory reasons
            ...
            if 'probTable' in original_npz.files:
                ...
                for m_idx in tqdm(range(M), desc="Applying component filter"):
                    for e_idx in range(E):
                        ...
                        # Use the new function to apply threshold and keep only the largest component
                        denoised_data[m_idx, e_idx, :, :] = keep_largest_component_from_threshold(
                            img=histogram_2d,
                            threshold=threshold,
                            fill_value=0
                        )
                ...
                for m_idx in tqdm(range(M), desc="Re-normalizing"):
                    for e_idx in range(E):
...
if __name__ == "__main__":
    ...
    if noisy is not None and denoised is not None:
        ...
        save_denoised_data(original_path=denoised_path, new_path=f'./denoised_output_advanced_mask_{method}_{threshold}.npz', threshold=threshold)
