...
class DenoisingUNet(nn.Module):
    def __init__(self, in_channels=5, out_channels=1, features_list=None):
        ...
        # Features list is now deeper and wider
        ...
        # --- Bottleneck ---
        self.bottleneck = ResidualBlock(features_list[3], features_list[4])
    ...
    def forward(self, x):
        ...
        if up1.shape[2:] != enc4_out.shape[2:]:
            up1 = nn.functional.interpolate(up1, size=enc4_out.shape[2:], mode='nearest')
        ...
        if up2.shape[2:] != enc3_out.shape[2:]:
            up2 = nn.functional.interpolate(up2, size=enc3_out.shape[2:], mode='nearest')
        ...
        if up3.shape[2:] != enc2_out.shape[2:]:
            up3 = nn.functional.interpolate(up3, size=enc2_out.shape[2:], mode='nearest')
        ...
        if up4.shape[2:] != enc1_out.shape[2:]:
            up4 = nn.functional.interpolate(up4, size=enc1_out.shape[2:], mode='nearest')
