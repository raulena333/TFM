...
def simulateBatchParticlesVectorized(
    ...
    while np.any(active):
        ...
        if interp:
            realAngles = np.empty(n)
            realEnergies = np.empty(n)
            for i in range(n):
                realAngles[i], realEnergies[i] = sampleReverseCalculateInterpolation(
                    data, material, energyActive[i], angleRange, energyRange, materialToIndex
                )
        else:
        ...
        near_zero = norms[:, 0] < 1e-8
        perp[near_zero] = np.array([1.0, 0.0, 0.0])
        perp[~near_zero] /= norms[~near_zero]
    ...
    return finalAngles, energyChanges
...
def runMultiprocessedBatchedSim(
    ...
    with Pool(processes=numWorkers) as pool:
        for angles, energies in tqdm(pool.imap(simulateBatch, argsList), total=len(argsList)):
            n = len(angles)
