...
def scatteringStepMaterial(
    ...
    if np.any(stayedSingleMaterial):
        ...
        newPositions[singleIndices] += stepLength * directions[singleIndices]
        ...
        depositEnergy3DStepTraversal(
            ...
            initialEnergies=sampledEnergies[singleIndices],
    ...
    if np.any(crossedMultipleMaterials):
        ...
        newPositions[multipleIndices] += lengthInFirstMaterial[:, None] * directions[multipleIndices]
        ...
        depositEnergy3DStepTraversal(
            ...
            energyDepositedVector=energyGrid,
            initialEnergies=sampledEnergies[multipleIndices],
            fluenceVector=fluenceGrid,
            energyFluenceVector=energyFluenceGrid
        )
...
def simulateBatchParticlesVectorized(
    ...
    initialZ = -0.5  # Initial position in mm
...
def simulateBatchParticlesWorker(args):
    ...
    result = simulateBatchParticlesVectorized(
        ...
        fluenceVector=fluence,
        energyFluenceVector=energyFluence,
        cdfs=cdfs,
        binEdges=binEdges,
        method=method,
        angleStep=angleStep,
    ...
    # Logging
    total_energy = np.sum(energyDepositedVector)
    nonzero_voxels = np.sum(energyDepositedVector > 0)

    log_message = f"Batch {batchID:04d} | Deposited Energy: {total_energy:.2f} MeV | Nonzero Deposits: {nonzero_voxels}\n"
...
def sampleAndPlotSingleEnergy(
    ...
    # --- PLOT sampled angle distribution ---
    plt.figure(figsize=(7.25, 6))
    sns.histplot(sampledAngles, bins=100, edgecolor= "black", color='darkred', kde=False)
    plt.xlabel("Angle (º)")
    plt.yscale('log')
    plt.title(f"Sampled angles - Material {materialIndex}, E = {energy} MeV, Method: {method}")
    plt.tight_layout()
    if saveFig:
        plt.savefig(f"SamplingAngles{method}_G{materialIndex}_ENERGY{energy}.pdf")
    plt.close()

    # --- PLOT sampled energy loss distribution ---
    plt.figure(figsize=(7.25, 6))
    sns.histplot(sampledEnergies, bins=100, edgecolor= "black", color='darkred', kde=False)
    plt.xlabel("Final energy (MeV)")
    plt.yscale('log')
    plt.title(f"Sampled energy- Material {materialIndex}, E = {energy} MeV, Method: {method}")
    plt.tight_layout()
    if saveFig:
        plt.savefig(f"SamplingEnergy{method}_G{materialIndex}_ENERGY{energy}.pdf")
    plt.close()
    
    print(f'Mean energy :' , np.mean(sampledEnergies))
    print(f'Std energy :' , np.std(sampledEnergies))
...
if __name__ == "__main__":
    ...
    # Shared settings
    samplingN = 10_000_000
    initialEnergy = 25.  # MeV
    bigVoxelSize = np.array((5., 5., 0.5), dtype=np.float32) # in mm
    voxelShapeBins = np.array((10, 10, 1), dtype=np.int32)
