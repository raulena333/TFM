...
if __name__ == "__main__":
    ...
    numpyPathLinear = "./NumpyInterpolationTransformation/"
    numpyPathCubic = "./NumpyCubicTransformation/"
    numpyPathNo = "./Numpy/"
    savePath = "./Plots/"
    linearInterpolation = f'{numpyPathLinear}projectionXZSimulation_transformation01.npy'
    cubicInterpolation = f'{numpyPathCubic}projectionXZSimulation_transformation02.npy'
    fileNameSimulation = f'{numpyPathNo}projectionXZSimulation_transformation.npy'
    ...
    dataTOPAS = np.load(fileNameTOPAS)
    dataSimulationTrans1 = np.load(linearInterpolation)
    dataSimulationTrans2 = np.load(cubicInterpolation)
    dataSimulation = np.load(fileNameSimulation)
    
    differenceDataTrans1 = dataTOPAS - dataSimulationTrans1
    differenceDataTrans2 = dataTOPAS - dataSimulationTrans2
    ...
    indxCenterSim = dataSimulationTrans1.shape[0] // 2
    profileZTopas = dataTOPAS[indxCenter, :, :]
    profileZSimulationTrans1 = dataSimulationTrans1[indxCenterSim, :, :]
    profileZSimulationTrans2 = dataSimulationTrans2[indxCenterSim, :, :]
    profileZSimulation = dataSimulation[indxCenterSim, :, :]
    
    profileZMeanTopas = profileZTopas.mean(axis=0)
    profileZMeanSimulationTrans1 = profileZSimulationTrans1.mean(axis=0)
    profileZMeanSimulationTrans2 = profileZSimulationTrans2.mean(axis=0)
    ...
    ax3.plot(zRange, profileZMeanSimulation, color='blue', linestyle='--', linewidth=1, label="No interpolation")
    ax3.plot(zRange, profileZMeanSimulationTrans1, color='darkorange', linestyle='--', linewidth=1, label="Linear Interpolation")
    ax3.plot(zRange, profileZMeanSimulationTrans2, color='green', linestyle='--', linewidth=1, label="Cubic Interpolation")
    ...
    diffZTrans1 = profileZMeanTopas - profileZMeanSimulationTrans1;
    diffZTrans2 = profileZMeanTopas - profileZMeanSimulationTrans2;
    diffZTrans = profileZMeanTopas - profileZMeanSimulation

    inset_ax3.plot(zRange, diffZTrans, color='blue', linestyle='-', linewidth=1, label='')
    inset_ax3.plot(zRange, diffZTrans1, color='darkorange', linestyle='-', linewidth=1, label='')
    inset_ax3.plot(zRange, diffZTrans2, color='green', linestyle='-', linewidth=1, label='')
