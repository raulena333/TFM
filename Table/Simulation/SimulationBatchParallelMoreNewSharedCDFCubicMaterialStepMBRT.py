...
def simulateBatchParticlesVectorized(
    ...
    numMinibeamsPerSide = 10  # Number of minibeam centers per side x and y
    minibeamsTotal = numMinibeamsPerSide ** 2
    minibeamSpacing = 3.0        # mm center-to-center spacing
    protonsPerMinibeam = batchSize // minibeamsTotal
    minibeamWidth = 0.1667           # mm (standard deviation for Gaussian spread, 3xsigma = +-0.5 mm)
    
    # Create grid coordinates for minibeam centers
    ...
    # Generate protons per minibeam
    xAll, yAll = [], []

    for xc, yc in zip(xCenters, yCenters):
        xVals = np.random.normal(loc=xc, scale=minibeamWidth, size=protonsPerMinibeam)
        yVals = np.random.normal(loc=yc, scale=minibeamWidth, size=protonsPerMinibeam)
        xAll.append(xVals)
        yAll.append(yVals)
    ...
    z = np.full_like(x, initialZ)
    position = np.stack([x, y, z], axis=1)  # Shape: (batchSize, 3)
