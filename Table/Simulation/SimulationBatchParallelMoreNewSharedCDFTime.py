...
# pyinstrument SimulationBatchParallelMoreNewSharedCDFTime.py --transformation -o report.html
# pyinstrument --load-prev ...
...
if __name__ == "__main__":
    ...
    if method == 'normalization':
        shm_bin_edges, bin_edges_shape, bin_edges_dtype = createSharedMemory(binEdges)
        kwargs.update(dict(
            shm_bin_edges=shm_bin_edges,
            bin_edges_dtype=bin_edges_dtype,
    ...
    # Call worker function directly for a single batch run (replace with your multiprocessing call if needed)
    simulateBatchParticlesWorker(tuple(args))
    
    profiler.disable()
    profiler.dump_stats(f'simulationProfile_{method}.out')
