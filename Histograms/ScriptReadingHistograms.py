...
def normalizeVariable(x, x_min = 0, x_max = -0.47):
    """
    Normalize x to the range [-1, 1] using the given x_min and x_max.
    """
    y = 2 * (x - x_min) / (x_max - x_min) - 1
    return y

def inverse_normalize_variable(y, x_min, x_max):
    """
    Revert the normalization of y back to x using the given x_min and x_max.
    """
    x = (y + 1) / 2 * (x_max - x_min) + x_min
    return x
